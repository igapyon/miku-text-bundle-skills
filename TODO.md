...
- Upstream main application anchor is `miku-text-bundle` release `v1.6.0`.
- Java companion runtime anchor is `miku-text-bundle-java` release `v1.6.0`.
...
- Node v1.6.0 and Java v1.6.0 both support knowledge-source execution.
...
- Final local release build targets `bundle/igapyon-miku-text-bundle-skills-1.6.0.zip`.
...
- [x] Identify the target releases: Node `miku-text-bundle` is `v1.6.0`; Java `miku-text-bundle-java` is `v1.6.0`.
...
- [ ] Create the release tag `v1.6.0` after final version confirmation and local verification.
