...
- Upstream main application anchor is `miku-text-bundle` release `v1.4.0`.
- Java companion runtime anchor is `miku-text-bundle-java` release `v1.4.0`.
...
- Final local release build currently generates `bundle/igapyon-miku-text-bundle-skills-1.4.0.zip`.
...
- [x] Identify the target Node `miku-text-bundle` release and Java `miku-text-bundle-java` release: both are `v1.4.0`.
...
- [ ] Create the release tag `v1.4.0` after final version confirmation and local verification.
