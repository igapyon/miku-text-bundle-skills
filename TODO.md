...
- Upstream main application anchor is `miku-text-bundle` release `v1.3.0`.
- Java companion runtime anchor is `miku-text-bundle-java` release `v1.3.0`.
...
- Final local release build currently generates `bundle/igapyon-miku-text-bundle-skills-1.3.0.zip`.
...
- [x] Identify the target Node `miku-text-bundle` release and Java `miku-text-bundle-java` release: both are `v1.3.0`.
...
- [x] Reinstall or rebuild the local Codex skill from the generated bundle.
- [x] Create the release tag `v1.3.0`.
